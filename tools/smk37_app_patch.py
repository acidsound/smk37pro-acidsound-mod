#!/usr/bin/env python3
"""Inspect and repack the SMK-37 Pro v12 application area safely.

This deliberately accepts only the byte-exact official v12 FWSC package.  It
...
OFFICIAL_V12_SIZE = 701_140
OFFICIAL_V12_PAYLOAD_SHA256 = (
    "23e65eb292ad8b3039085f3fa27ee738b5fa7d56ac02611c2908f6136858af9a"
)
OFFICIAL_V12_PAYLOAD_SIZE = 701_120
FWSC_BLOCK_SIZE = 48
FWSC_DATA_SIZE = 47
FWSC_SLOTS = 20
...
def decode_fwsc_metadata(raw: bytes) -> tuple[str, int]:
    ...
    require(decoded.startswith(b"SMK-37 Pro_012"), "unexpected FWSC product/version metadata")
    return "SMK-37 Pro", 12


def unpack_fwsc(raw: bytes) -> tuple[bytearray, bytes]:
    require(len(raw) == OFFICIAL_V12_SIZE, "official v12 FWSC size mismatch")
    require(sha256(raw) == OFFICIAL_V12_SHA256, "input is not the byte-exact official v12 FWSC")
    ...
    require(len(payload) == OFFICIAL_V12_PAYLOAD_SIZE, "FWSC payload size mismatch")
    require(sha256(payload) == OFFICIAL_V12_PAYLOAD_SHA256, "official v12 payload hash mismatch")
    return payload, metadata


def repack_fwsc(original: bytes, payload: bytes | bytearray, metadata: bytes) -> bytes:
    require(len(payload) == OFFICIAL_V12_PAYLOAD_SIZE, "repacked payload size changed")
...
class Ufw:
    ...
    def flash_entry(self) -> UfwEntry:
        ...
        entry = matches[0]
        require(entry.offset == 0x400 and entry.size == FLASH_SIZE, "unexpected v12 flash.bin layout")
        return entry
...
def build_package(original: bytes, replacement_app: bytes) -> tuple[bytes, dict[str, object]]:
    ...
    manifest: dict[str, object] = {
        "format": "smk37-v12-application-only-patch-v1",
        "safety_gate": "PASS",
        "input": {
            "size": len(original),
...
def inspect_package(raw: bytes) -> dict[str, object]:
    ...
    return {
        "format": "smk37-v12-inspection-v1",
        "safety_gate": "PASS",
...
def self_test() -> None:
    ...
    print("smk37_app_patch self-test: PASS")
...
def main() -> int:
    ...
    inspect_parser = subparsers.add_parser("inspect", help="validate and describe official v12")
    inspect_parser.add_argument("input", type=Path)
    inspect_parser.add_argument("--manifest", type=Path)

    extract_parser = subparsers.add_parser(
        "extract-app", help="extract the byte-exact app.bin from official v12"
    )
    ...
    app_parser = subparsers.add_parser(
        "repack-app",
        help="repack one exact-size replacement app.bin into official v12",
    )

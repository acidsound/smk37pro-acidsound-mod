#!/usr/bin/env python3
"""Build the offline-only guarded official-v15 rollback bundle for H0.

The output restores only the two sectors that differ between exact official v15
and the exact H0 heap-boundary diagnostic package. It copies the already reviewed
R02 transport/restore implementation, then tightens identifiers, confirmations,
and sector allow-list to H0. This tool never opens USB or writes Flash.
"""
...
EXPECTED_SECTORS = (0x04000, 0x62000)
SECTOR_SIZE = 0x1000
BUNDLE_NAME = "SMK37Pro-WL82-v15-H0-rollback-20260802-v1"
...
def patched_guard(template: str) -> str:
    replacements = (
        ("v15 R02 rollback bundle", "v15 H0 rollback bundle"),
        ("exact four 4 KiB sectors", "exact two 4 KiB sectors"),
        ("{0x04000,0x20000,0x22000,0x2A000}", "{0x04000,0x62000}"),
        ("SECTORS = {0x04000, 0x20000, 0x22000, 0x2A000}", "SECTORS = {0x04000, 0x62000}"),
        ...
        ("smk37-v15-r02-forced-recovery-plan-v1", "smk37-v15-h0-forced-recovery-plan-v1"),
        ("expected R02 hash", "expected H0 hash"),
        ("pre-erase R02 hash", "pre-erase H0 hash"),
        ("expected R02 target-sector hashes", "expected H0 target-sector hashes"),
        ("R02 rollback", "H0 rollback"),
        ("R02 hash", "H0 hash"),
        ("R02", "H0"),
        ("r02", "h0"),
    )
    ...
    require("R02" not in result and "r02" not in result, "stale R02 token in H0 guard")
    require("0x20000" not in result and "0x22000" not in result and "0x2A000" not in result,
            "H0 guard still allows non-H0 sectors")
    require("0x62000" in result and "0x04000" in result, "H0 guard lacks exact sector allow-list")
    for confirmation in CONFIRMATIONS:
        require(confirmation in result, f"H0 guard missing confirmation: {confirmation}")
    return result
...
def patched_wrapper(template: str) -> str:
    ...
    for old, new in replacements:
        if old in result:
            result = result.replace(old, new)
    require("R02" not in result and "r02" not in result, "stale R02 token in H0 elevated wrapper")
    for confirmation in CONFIRMATIONS:
        require(result.count(f"--confirm {confirmation}") == 1,
                f"H0 elevated wrapper confirmation mismatch: {confirmation}")
    return result
...
def main() -> int:
    ...
    require(args.output_dir.name == BUNDLE_NAME, "unexpected H0 bundle directory name")
    require(not args.output_dir.exists(), "refusing to overwrite existing H0 rollback directory")
    require(not args.output_zip.exists(), "refusing to overwrite existing H0 rollback ZIP")
    ...
    require(digest(args.target.read_bytes()) == H0_PACKAGE_SHA256, "H0 package hash mismatch")
    ...
    require(sectors == EXPECTED_SECTORS, f"unexpected H0 sector set: {sectors}")
    ...
    manifest = {
        "format": "smk37-v15-h0-forced-recovery-plan-v1",
        ...
        "safety_policy": {
            ...
            "required_before_write": "two identical fresh 1 MiB forced-loader dumps and exact H0 target-sector hashes",
            "write_scope": "two audited 4 KiB application sectors only",
        ...
        "h0_package_sha256": H0_PACKAGE_SHA256,
        "stock_flash_sha256": digest(stock),
        "h0_flash_sha256": digest(target),
    ...
    for path in (p for p in args.output_dir.rglob("*") if p.is_file()):
        ...
        require(b"R02" not in data and b"r02" not in data,
                f"stale R02 token in H0 bundle: {path.relative_to(args.output_dir)}")

    completed = subprocess.run([sys.executable, str(guard_path), "self-test"], check=False)
    require(completed.returncode == 0, "H0 guarded restore self-test failed")
    write_zip(args.output_dir, args.output_zip)
    print("H0 rollback bundle", digest(args.output_zip.read_bytes()))

...
>
> **T1/T2 결과 (2026-08-14)**: 에디터에서 **Original과 all-C4(60) 모두 변경 없음**
> ("어느쪽을 해도 변경점이 없음"). 같은 all-C4(60)가 직접 USB에서는 로드 성공했으므로
> → **에디터 Web MIDI 경로가 문제**로 확정, byte 161 값과 무관.
> 원인: OTA 시도 중 MIDIServer를 재시작(2회)해 Chrome의 CoreMIDI 세션이
> 끊어진 상태 — "Web MIDI 재연결" 버튼만으로는 복구되지 않을 수 있음.
> 복구: 에디터 탭 완전히 닫고 재오픈(새 MIDIAccess) → Web MIDI 연결 → 장치
> 드롭다운에서 SMK-37 Pro Midi 확인 → 활동 로그에 "Sent 1/16 … 16/16"
> 확인 → all-C4 재테스트.
>
> **값 범위 반증 (2026-08-14, 직접 USB 대조)**: 브라우저를 우회한 libusb
> 센더로 ① all-C4(byte 161 = 60) ② Original(byte 161 = 36..51, Bank D
> 템플릿 복원) ③ FM identity-safe(byte 161 = 36..51, FM 키트 16음) 세 가지
> 모두 **전송 에러 0**이며, ①과 ②는 사용자 확인 **로드 성공** (③은 확인 중).
> → **byte 161 값 범위(36..51 vs 60)는 원인이 아님**. 장치·패킷 포맷·USB
> 경로 모두 정상이며, 관찰된 모든 실패는 에디터 Web MIDI 세션 문제로
> 설명됩니다. "36..51이 로드를 깨뜨린다"는 가설은 **폐기**.
> 참고: 직접 USB 전송은 매번 MIDIServer 재시작이 필요해 브라우저 세션을
> 끊으므로, 에디터 재검증 시 Chrome 완전 재시작 후 진행.
